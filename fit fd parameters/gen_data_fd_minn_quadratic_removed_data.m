function P = gen_data_fd_minn_quadratic_removed_data(rhom)
...
N2=length(initialrm);
N=N1*N2;
P = zeros(N,5);  % initialize the matrix
%YE GARZ-----------------------------------------------------------------
...
for iter1 = 1:N1
    for iter2 = 1:N2
        ...
        x0 = [initialvm(iter1) initialrm(iter2)];
        options = optimoptions('fmincon','MaxFunEvals', 1*10^10, 'MaxIter', 1*10^5, 'TolX',1*10^(-15), 'TolCon',1*10^(-15));
        %options = optimoptions('fmincon','MaxFunEvals', 1*10^10, 'MaxIter', 1*10^5,);
        %options = optimset('MaxFunEvals', 1*10^10, 'MaxIter', 1*10^5);
        %options = optimset('MaxFunEvals', 1*10^10);
        [x,fval] = fmincon(@(x)beta(1)*norm(max((traffic_flux_qrd(density,x(1),x(2))...
            -flux),0*flux))^2+(1-beta(1))*norm(max(-(traffic_flux_qrd(density,x(1),x(2))...
            -flux),0*flux))^2,x0,[],[],[],[],lb,ub,[],options);
...
savefile = sprintf('Gpt_varyingIC-0411');
...
grid = 0:P(1,2);    % plot of the sequence of flow-density curves
...
%---------------------------------------------------------
%  4 parameters
 plot(grid,traffic_flux_qrd(grid,P(1,1),P(1,2)),'b-','linewidth',2.5),hold on
