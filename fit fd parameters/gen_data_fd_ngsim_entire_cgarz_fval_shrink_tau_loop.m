function P = gen_data_fd_ngsim_entire_cgarz_fval_shrink_tau_loop
...
% beta = [h,.5,1-h];
% % test beta data
beta = [.01,.02,.03,.05,.1,.3,.5,0.9,0.99];
                ...
                for iter5=1:N5
                    for iter6=1:N6
                        ...
                        [x,fval] = fmincon(@(x) beta(9)*(norm(max(shrinkage(density,(func_fd_seibold_2p_updated(density,x(1),x(2),x(7),x(8),x(9),x(10))...
                            ...
                            -flux),x(7),tau_set(iter0)),0*flux)))^2,x0,[],[],[],[],lb,ub);%CGARZ!!!
...
savefile = sprintf('tau_rho0_ngsim_800_1100');
