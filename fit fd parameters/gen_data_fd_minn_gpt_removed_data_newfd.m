function P = gen_data_fd_minn_gpt_removed_data_newfd(rhom)
...
%                     -flux),0*flux))^2,x0,A,b,[],[],lb,[],[],options);
                ...
                [x,fval] = fmincon(@(x)beta(eq_id)*norm(max((func_flux_phase_transition(density,x(4),x(1),x(2),x(3))...
                    -flux),0*flux))^2+(1-beta(eq_id))*norm(max(-(func_flux_phase_transition(density,x(4),x(1),x(2),x(3))...
...
savefile = sprintf('GPT_varyingIC_real_fval-NonUb-0406-newfd');
...
%---------------------------------------------------------
%--------------------------------------------------------
 plot(grid,func_flux_phase_transition(grid,P(1,4),P(1,1),P(1,2),P(1,3)),'m--','linewidth',4),hold on
%--------------------------------------------------------
