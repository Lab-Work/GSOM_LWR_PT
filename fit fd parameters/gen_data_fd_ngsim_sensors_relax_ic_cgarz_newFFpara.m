function P = gen_data_fd_ngsim_sensors_relax_ic_cgarz_newFFpara
...
N=N1*N2*N3;
m = length(beta);
P = zeros(N,2*(m+2)+m);  % initialize the matrix
% P = zeros(m,3);  % initialize the matrix
%YE CGARZ-------------------------------------------------------
...
for iter1 = 1:N1
    for iter2 = 1:N2
        for iter3 = 1:N3
            ...
            P(count,m*2+3) = initialr(iter3);
            P(count,13) = x(1);
            P(count,14) = x(2);
            P(count,m*2+4) = x(3);
            rho_m = x(3);
            rho_m
            P(count,2*(m+2)+7)=fval;
...
savefile = sprintf('CGARZ_varyingIC-realistic_rhom0_fval-ngsim');
...
plot(grid,func_fd_seibold_2p(grid,P(1,3),P(1,4),rho0,v0,f0,uf,rhof,P(1,22)),'k-','linewidth',4), hold on
