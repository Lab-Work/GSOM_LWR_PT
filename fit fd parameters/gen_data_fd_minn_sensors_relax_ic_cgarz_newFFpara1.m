...
N=N1*N2;
m = length(beta);
P1 = zeros(N,2*6+2+1);  % initialize the matrix
% P = zeros(m,3);  % initialize the matrix
%YE CGARZ-------------------------------------------------------
    ...
    for iter2 = 1:N2
            ...
            for j = 1:m
                ...
                if j~=7 && j~=1 && j~=9
                    ...
                    P1(count,(j-1)*2+1) = x(1);
                    P1(count,(j-1)*2+2) = x(2);
                    P1(count,13)=initials(iter1);
                    P1(count,14)=initialm(iter2);
                    P1(count,15)=fval;
                    count
                    j
                end
...
savefile = sprintf('CGARZ_varyingIC-realistic_rhom0_fval-temp');
% savefile = sprintf('data_fd_minn_garz');
% 
save(savefile,'P1');    % P is the parameters, n cross 3
...
plot(grid,func_fd_seibold_2p(grid,P1(1,3),P1(1,4),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
plot(grid,func_fd_seibold_2p(grid,P1(1,5),P1(1,6),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
plot(grid,func_fd_seibold_2p(grid,P1(1,7),P1(1,8),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
plot(grid,func_fd_seibold_2p(grid,P1(1,9),P1(1,10),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
plot(grid,func_fd_seibold_2p(grid,P1(1,11),P1(1,12),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
%plot(grid,func_fd_seibold_2p(grid,P1(1,13),P1(1,14),rho0,v0,f0,uf,rhof,P1(1,22)),'m-','linewidth',4), hold on
plot(grid,func_fd_seibold_2p(grid,P1(1,15),P1(1,16),rho0,v0,f0,uf,rhof,rho_m),'k-','linewidth',4), hold on
