...
for i =1:9
    load GARZ_varyingIC_realistic_rhom01.mat
        figure(i)
        lambda = P_current(1,(i-1)*3+1);
        p = P_current(1,(i-1)*3+2);
        alpha = P_current(1,(i-1)*3+3);
        rhom=P_current(1,32);
        count=0;
        for j=1:162;
            strld = ['GARZ_varyingIC_realistic_rhom0',num2str(j),'.mat']
            load(strld);
            
            if (P_current(1,(i-1)*3+1)<lambda+50 && P_current(1,(i-1)*3+1)>lambda-50 && P_current(1,(i-1)*3+2)<p+50 && P_current(1,(i-1)*3+2)>p-50 && P_current(1,(i-1)*3+3)<alpha+50 && P_current(1,(i-1)*3+3)>alpha-50 && P_current(1,32)<rhom+50 && P_current(1,32)>rhom-50)
                count=count+1;
                scatter3(P_current(1,(i-1)*3+1),P_current(1,(i-1)*3+2),P_current(1,(i-1)*3+3),'m'),hold on 
                set(gca,'Xscale','log','Zscale','log','Yscale','log')
%                    scatter(j,P_current(1,32),'m'),hold on
%                    set(gca,'Yscale','log')
            else
%               if (P_current(1,(i-1)*3+1)<10000 && P_current(1,(i-1)*3+2)<1 && P_current(1,(i-1)*3+3)<10000)
                   scatter3(P_current(1,(i-1)*3+1),P_current(1,(i-1)*3+2),P_current(1,(i-1)*3+3),'b'),hold on
                   set(gca,'Xscale','log','Zscale','log','Yscale','log')
%               end
% %                  if (P_current(1,32)<10000)
%                   scatter(j,P_current(1,32),'b'),hold on
%                   set(gca,'Yscale','log')
% %                  end
            end
        end
...
%         strt = ['Parameter-GARZ ',' \beta = ',num2str(beta(i)),' count=',num2str(count),'/',num2str(882)];
%         strts = ['Parameter-GARZ-log-',num2str(i),'.fig'];
