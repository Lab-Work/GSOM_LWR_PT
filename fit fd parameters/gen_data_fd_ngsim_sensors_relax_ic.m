function P = gen_data_fd_minn_sensors_relax_ic(rhom)
            ...
            for iter4 =1:N4
                ...
                for j = 1:m
                    if j~=7
                        ...
                        P(count,(j-1)*3+2) = x(2);
                        P(count,(j-1)*3+3) = x(3);
                        P(count,m*3+5+j) = fval;
                        P_current=P(count,:)
%                         strsv = ['GARZ_varyingIC_realistic_rhom0_fval',num2str(count)]
...
savefile = sprintf('GARZ_varyingIC-realistic_rhom0_fval_ngsim');
