function P = gen_data_fd_minn_entire_cgarz_freq_shrink_removed_data_tau
...
% savefile = sprintf('CGARZ_entire_fval-rhof-1-minn-0329');
...
load CGARZ_entire_fval-rhof-1-minn-0329-tau400.mat;

index_rf = find(P(:,19)>1);
P=P(index_rf,:);
...
for j=1:3
    ...
    for i =1:(length(P_rm_0(:,1))-1)
        if P_sort(i,18+j)>P_sort(i+1,18+j)-1 && P_sort(i,18+j)<P_sort(i+1,18+j)+1
            count=count+1;
...
for iter=1:9
    ...
    if length(P_rm(:,1))~=1
        ...
        if iter==1 || iter==9
            plot(grid,func_fd_seibold_2p_updated(grid,para(iter,1),para(iter,2),rho0,uf,rhof,rho_m),'r-','linewidth',2.5),hold on
        elseif iter==7
...
savefile = sprintf('CGARZ_entire_freq_rhof-2-ngsim');
% savefile = sprintf('data_fd_minn_garz');
% 
...
text(350,10000,strr,'fontsize',14);
strrm = ['\rho_{m} = ',num2str(rho_m)];
text(350,8000,strrm,'fontsize',14);
