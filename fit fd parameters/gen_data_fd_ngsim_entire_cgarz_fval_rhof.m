function P = gen_data_fd_ngsim_entire_cgarz_fval_rhof
                ...
                for iter5=1:N5
                    for iter6=1:N6
                        ...
                        [x,fval] = fmincon(@(x) ((1/(3*max(length(density))))*(beta(9)*(norm(max((func_fd_seibold_2p_updated(density,x(1),x(2),x(7),x(8),x(9),x(10))...
                            ...
                            -flux),0*flux)))^2))^(0.5)-0.1*x(7),x0,[],[],[],[],lb,ub);%CGARZ!!!
...
savefile = sprintf('CGARZ_entire_fval-rhof-1-ngsim');
...
savefile = sprintf('CGARZ_entire_fval-rhof-2-ngsim');
% savefile = sprintf('data_fd_minn_garz');
% 
