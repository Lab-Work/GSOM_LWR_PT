function P = gen_data_fd_minn_entire_cgarz_freq_rhof
...
% savefile = sprintf('CGARZ_entire_freq-1');
...
load CGARZ_entire_fval-rhof-1-minn.mat;
...
% initials = [100,500,1000];
% initialm = [50,100,1000];
initials = [100];
initialm = [100];
% initialr = [500,600];
N1 = length(initials);
    ...
    for iter2 = 1:N2       
        ...
        for j = 1:m         
            if j~=7 && j~=1 && j~=9
                ...
                %        options = optimset('MaxFunEvals', 1*10^10, 'MaxIter', 1*10^5);
                %options = optimset('MaxFunEvals', 1*10^10);
                [x,fval] = fmincon(@(x) beta(j)*(norm(max((func_fd_seibold_2p_updated(density,x(1),x(2),rho0,uf,rhof,rho_m)...
                    -flux),0*flux)))^2+(1-beta(j))*(norm(max(-(func_fd_seibold_2p_updated(density,x(1),x(2),rho0,uf,rhof,rho_m)...
