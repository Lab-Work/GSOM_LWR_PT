function P = gen_data_fd_minn_gpt_removed_data(rhom)
...
N=N1*N2*N3*N4;
m = length(beta);
P = zeros(N,8);  % initialize the matrix
    ...
    for iter2 = 1:N2
        for iter3 = 1:N3
            for iter4 =1:N4
                ...
                %options = optimset('MaxFunEvals', 1*10^10, 'MaxIter', 1*10^5);
                [x,fval] = fmincon(@(x)beta(eq_id)*norm(max((func_flux_creen_pt(density,0,x(1),x(2),x(3))...
                    -flux),0*flux))^2+(1-beta(eq_id))*norm(max(-(func_flux_creen_pt(density,0,x(1),x(2),x(3))...
                    -flux),0*flux))^2,x0,A,b,[],[],lb,[],[],options);

%                 [x,fval] = fmincon(@(x)beta(eq_id)*norm(max((func_flux_phase_transition(density,0,x(1),x(2),x(3))...
%                     -flux),0*flux))^2+(1-beta(eq_id))*norm(max(-(func_flux_phase_transition(density,0,x(1),x(2),x(3))...
%                     -flux),0*flux))^2,x0,[],[],[],[],lb,[],[],options);
                ...
                P(count,2) = x(2);
                P(count,3) = x(3);
                P(count,4) = initialrc(iter1);
                P(count,5) = initialvm(iter2);
                P(count,6) = initialrm(iter3);
                P(count,7) = initialq(iter4);
                P(count,8) = fval;
...
savefile = sprintf('GPT_varyingIC_real_fval-NonUb-0406');
