function P = gen_data_fd_minn_cgarz_shrink_removed_data
...
N=N1*N2*N3*N4*N5*N6;
m = length(beta);
P = zeros(N,17);  % initialize the matrix
% P = zeros(m,3);  % initialize the matrix
%YE CGARZ-------------------------------------------------------
...
% % P = zeros(m,3);  % initialize the matrix
%YE GARZ-----------------------------------------------------------------
tau=300;%%currently tau=300 is the best
            ...
            for iter4 =1:N4
                for iter5=1:N5
                    for iter6=1:N6
                        ...
                        [x,fval] = fmincon(@(x) beta(3)*(norm(max(shrinkage(density,(func_fd_seibold_2p_updated(density,x(1),x(2),x(7),x(8),x(9),x(10))...
                            -flux),x(7),tau),0*flux)))^2+(1-beta(3))*(norm(max(shrinkage(density,-(func_fd_seibold_2p_updated(density,x(1),x(2),x(7),x(8),x(9),x(10))...
                            -flux),x(7),tau),0*flux)))^2+beta(2)*(norm(max(shrinkage(density,(func_fd_seibold_2p_updated(density,x(3),x(4),x(7),x(8),x(9),x(10))...
                            -flux),x(7),tau),0*flux)))^2+(1-beta(2))*(norm(max(shrinkage(density,-(func_fd_seibold_2p_updated(density,x(3),x(4),x(7),x(8),x(9),x(10))...
                        ...
                        P(count,11) = initials(iter1);
                        P(count,12) = initialm(iter2);
                        P(count,13) = initialr0(iter3);                                               
                        P(count,14) = initialuf(iter4);
                        P(count,15) = initialrf(iter5);
                        P(count,16) = initialr(iter6);
                        P(count,17) = fval;
...
savefile = sprintf('CGARZ_entire_varyingIC-0406-tau300');
...
% end
% 
% savefile = sprintf('CGARZ_entire_fval-rhof-2-minn-0329-1-tau400');
% % savefile = sprintf('data_fd_minn_garz');
% % 
