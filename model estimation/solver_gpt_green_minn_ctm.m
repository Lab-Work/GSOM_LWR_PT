% function [e_rho,e_vel,erho,evel] = solver_gpt_green_minn_ctm(tfinal,pos,D,Vel,...
%     h,w_min,w_max,rhoc,vm,rhom)
function [erho,evel] = solver_gpt_green_minn_ctm(tfinal,pos,D,Vel,...
    h,w_min,w_max,rhoc,vm,rhom)
...
function  F = Num_Flux(Ul,Ur,rhoc,vm,rhom) % numerical flux
   ...
   s_flux = sending(Ul,rhoc,vm,rhom);
   % receiving of flux
...
%       q = w.*rho_middle;
      ...
      q_max = traffic_flux_smooth(sigma,sigma.*w,rhoc,vm,rhom); % maximum flow rate
      y = case2.*rho_middle.*ur + case1.*q_max;  
%----------------------------------------------
% define sending functions
function y = sending(Ul,rhoc,vm,rhom)% U = [\rho, w] two state variables
      w = Ul(2,:)./Ul(1,:); % property on left side
      q = Ul(2,:);
      rho = Ul(1,:);
      ...
      q_max = traffic_flux_smooth(sigma,sigma.*w,rhoc,vm,rhom); % maximum flow rate
      flow = traffic_flux_smooth(rho,q,rhoc,vm,rhom);
      y = case1.*flow + case2.*q_max;
%-----------------------------------------------
