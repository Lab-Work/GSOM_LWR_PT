function [e_rho,e_vel]=comput_minn_validation_cluster_newptfd(day)
% if nargin<2 models = 2; resol = 1; proj = 1; end
if nargin<1 day = 69; end
clc;
resol = 7;
proj = 1;%or 2
   tic
...
ca_2p= polyfit(q(1:end),fd_collap(1:end,1),order1);
cb_2p = polyfit(q(1:end),fd_collap(1:end,2),order1);

grid1 = (min(q(1:end))):(max(q(1:end))); 

subplot(2,3,4);
plot(q(1:end),fd_collap(1:end,1),'.','color',[0.2,0.2,0.2],'markersize',6), hold on
plot(grid1, polyval(ca_2p,grid1) ,'b-','linewidth',1),hold off

subplot(2,3,5);
plot(q(1:end),fd_collap(1:end,2),'.','color',[0.2,0.2,0.2],'markersize',6), hold on
...
%                 rho_r,h,w_max,q_min,q_max,542.1229);
% % 
% % 
% % 
% % 
...
% 
% 
% %            [erho,evel] = solver_gpt_green_minn_ctm(tfinal,pos,B,V,h,-0.5954,0.32,80.8879,94.1755,545.8811);
% %             [erho,evel] = solver_gpt_daganzo_minn_ctm(tfinal,pos,B,V,h,-0.33,0.61,80.2,93.7,533.33)
% %            [erho,evel] = solver_gpt_green_minn_ctm(tfinal,pos,B,V,h,-0.33,0.61,80.2,93.7,133);
%   
% 
%             e_rho(8) = erho; % error from density
%             e_vel(8) = evel; % error from velocity 
